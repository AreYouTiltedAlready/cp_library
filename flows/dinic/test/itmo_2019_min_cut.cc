...
// https://codeforces.com/group/QmrArgR1Jp/contest/322857/problem/B
// https://codeforces.com/group/QmrArgR1Jp/contest/322857/submission/238806327
void RunCase([[maybe_unused]] int testcase) {
  int n;
  int m;
  std::cin >> n >> m;
 
  FlowGraph<int> FlowGraph(n, m, 0, n - 1);
  for (int i = 0; i < m; ++i) {
    int u;
    int v;
    int cap;
    ...
    --u;
    --v;
    FlowGraph.AddEdge(u, v, cap, cap);
  }
 
  const int max_flow = FlowGraph.DinicWithScaling();
  const std::vector<int> min_cut = FlowGraph.MinCut();
  std::cout << min_cut.size() << " " << max_flow << "\n";
  for (int id : min_cut) {
    std::cout << id / 2 + 1 << " \n"[id == min_cut.back()];
  }
}
