...
class DynamicModint {
public:
  static void SetMod(uint32_t mod) {
    kMod = mod;
    barrett = Barrett(mod);
  }
  ...
  DynamicModint(T value) : value_( // NOLINT(*-explicit-constructor)
      value % kMod) {
  ...
  DynamicModint(T value) : // NOLINT(*-explicit-constructor)
      value_(value % kMod) {}
...
private:
  static inline uint32_t kMod = 1;
  static inline Barrett barrett = Barrett(1);
  uint32_t value_;
};
