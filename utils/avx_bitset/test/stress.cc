...
template <int kSize>
std::pair<std::bitset<kSize>, bitset::avx_bitset> RandomBinaryString() {
  std::bitset<kSize> res_std;
  bitset::avx_bitset res(kSize);
...
template <int kIterations, int kSize>
void Test() {
  std::bitset<kSize> std_bitset;
  bitset::avx_bitset bitset(kSize);

  auto CheckResults = [&](auto&& lhs, auto&& rhs) -> void {
    if (lhs != rhs) {
      std::cerr << "FAILURE!\n";
      std::cerr << "Expected: " << lhs << "\n";
      std::cerr << "Actual: " << rhs << "\n";
      exit(0);
    }
  };
  ...
  auto Check = [&]() -> void {
    CheckResults(std_bitset.to_string(), bitset.to_string());
  };

  std::cerr << "Test suite start\n\n";
  for (int i = 0; i < kIterations; ++i) {
    std::cerr << "Query " << i + 1 << "\n";
    auto query = static_cast<Query>(static_cast<int>(gen() % 12));
    switch (query) {
      case Query::kOr: {
        std::cerr << "OR query:\n";
        auto [x, y] = RandomBinaryString<kSize>();
        //   std::cerr << "Random bitset: " << x.to_string() << "\n";
        std_bitset |= x;
        bitset |= y;
        Check();
      } break;
      ...
      case Query::kNone: {
        std::cerr << "None query:\n";
        CheckResults(std_bitset.none(), bitset.none());
      } break;
      case Query::kAny: {
        std::cerr << "Any query:\n";
        CheckResults(std_bitset.any(), bitset.any());
      } break;
      case Query::kAll: {
        std::cerr << "All query:\n";
        CheckResults(std_bitset.all(), bitset.all());
      } break;
      case Query::kCount: {
        std::cerr << "Count query:\n";
        CheckResults(std_bitset.count(), bitset.count());
      } break;
      case Query::kShiftLeft: {
        std::cerr << "Shift left query:\n";
        int s = static_cast<int>(gen() % (kSize + 1));
        std::cerr << "Shift: " << s << "\n";
        std_bitset >>= s;
        bitset >>= s;
        Check();
      } break;
      case Query::kShiftLeftCopy: {
        std::cerr << "Shift left with copying query:\n";
        int s = static_cast<int>(gen() % (kSize + 1));
        std::cerr << "Shift: " << s << "\n";
        std_bitset = std_bitset >> s;
        bitset = bitset >> s;
        Check();
      } break;
      case Query::kShiftRight: {
        std::cerr << "Shift right query:\n";
        int s = static_cast<int>(gen() % (kSize + 1));
        std::cerr << "Shift: " << s << "\n";
        std_bitset <<= s;
        bitset <<= s;
      ...
      case Query::kShiftRightCopy: {
        std::cerr << "Shift right with copying query:\n";
        int s = static_cast<int>(gen() % (kSize + 1));
        std::cerr << "Shift: " << s << "\n";
        std_bitset = std_bitset << s;
        bitset = bitset << s;
    ...
    // std::cerr << "OK\n";
    // std::cerr << "std:: " << std_bitset.to_string() << "\n";
    // std::cerr << "avx:: " << bitset.to_string() << "\n";
  }
}
