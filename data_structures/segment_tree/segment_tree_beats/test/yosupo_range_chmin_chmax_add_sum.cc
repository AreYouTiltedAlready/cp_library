...
// https://judge.yosupo.jp/submission/179648
void RunCase([[maybe_unused]] int testcase) {
  ...
  JiDriverSegmentTree<int64_t> segment_tree_beats(v);
  while (q--) {
    int t;
    int l;
    int r;
    std::cin >> t >> l >> r;
    if (t == 3) {
      std::cout << segment_tree_beats.Get(l, r).sum << "\n";
    } else {
      int64_t value;
      std::cin >> value;
      if (t == 0) {
        segment_tree_beats.ApplyChmin(l, r, value);
      } else if (t == 1) {
        segment_tree_beats.ApplyChmax(l, r, value);
      } else {
        segment_tree_beats.ApplyAdd(l, r, value);
      }
    }
  }
}
