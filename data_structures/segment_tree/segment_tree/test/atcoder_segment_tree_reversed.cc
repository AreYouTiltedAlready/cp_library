...
// Submission: https://atcoder.jp/contests/practice2/submissions/49020706
...
void RunCase([[maybe_unused]] int testcase) {
  ...
  while (q--) {
    ...
    if (t == 1) {
      ...
      last = n - last;
      std::swap(first, last);
      std::cout << segment_tree.Get(first, last + 1).value << "\n";
    } else {
      int first;
      int value;
      std::cin >> first >> value;
      int res = segment_tree.FindLast(0, n - first + 1, [value](const MaxS& z) {
