...
// Submission: https://judge.yosupo.jp/submission/181697
...
void RunCase([[maybe_unused]] int testcase) {
  ...
  using Mint = modular::modint::StaticMInt<998244353>;
  std::vector<Mint> p(n);
  for (Mint& it : p) {
    std::cin >> it;
  }
  ...
  const std::vector<Mint> r = ntt::Convolution(std::move(p), std::move(q));
  for (const Mint& it : r) {
    std::cout << it << " ";
  }
}
